...
func SetStrength(res http.ResponseWriter, req *http.Request, parms martini.Params) (int, string) {
	// strength must a value between 0 and
	strengthstr := parms["strength"]
	strength, _ := strconv.ParseInt(strengthstr, 10, 64)

	if strength < 0 || strength > 6 {
		return 400, "strength is not in the range"
	}

	fmt.Printf("strength: %d", strength)
